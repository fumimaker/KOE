...
void fadeChange(uint8_t R, uint8_t G, uint8_t B, uint16_t wait){
  boolean R_flg = false, G_flg = false, B_flg = false;
  ...
  while( (R_flg==true) || (G_flg==true) || (B_flg==true) ){//全ての値が目標値に到達していなかったら
    for(int16_t i = 0; i < strip.numPixels(); i++ ){//するまでやる
      ...
      /*tmp = color & 0xFF0000;
      uint16_t now_R = tmp >> 16;
      tmp = color & 0x00FF00;
      uint16_t now_G = tmp >> 8;
      tmp = color & 0x0000FF;
      uint16_t now_B = tmp;*/
    ...
    if(str == 0x20){
      ...
      while(quit_flg){
        ...
        for(i=0; i<strip.numPixels(); i++){
          ...
          if(i>0) strip.setPixelColor(i-1, 128, 128, 0);
          if(i>1) strip.setPixelColor(i-2, 64, 64, 0);
          if(i>2) strip.setPixelColor(i-3, 32, 32, 0);
          if(i>3) strip.setPixelColor(i-4, 16, 16, 0);
          if(i>4) strip.setPixelColor(i-5, 8, 8, 0);
          if(i>5) strip.setPixelColor(i-6, 4, 4, 0);
          if(i>6) strip.setPixelColor(i-7, 2, 2, 0);
          if(i>7) strip.setPixelColor(i-8, 0, 0, 0);
          delay(25);
      ...
      case '6':{//一人一人挨拶
        ...
        while(quit_flg){
          ...
          if(Serial.read()==0x20){
            ...
            else if( num == 1 ) colorWipe(strip.Color(  157,  204,  254 ), 3);//あゆみ
            else if( num == 2 ) colorWipe(strip.Color(    0,  255,    0 ), 3);//ぐっちー
            else if( num == 3 ) colorWipe(strip.Color(  135,    0,  204 ), 3);//いずみ
            else if( num == 4 ) colorWipe(strip.Color(  255,   55,    0 ), 3);//まーしー
            else if( num == 5 ) colorWipe(strip.Color(  255,    0,  255 ), 3);//かなみ
            else if( num == 6 ) colorWipe(strip.Color(    0,    0,  255 ), 3);//つばさ
            else if( num == 7 ) colorWipe(strip.Color(  255,    0,    0 ), 3);//ねぐせ
            else if( num == 8 ){
  ...
  while(mode_flg){
    ...
    switch(str){
      ...
      case '4':{//I`ll going back
        for(i=COLUMN1-ONLEDNUM; i<COLUMN1; i++){
          strip.setPixelColor(i, 255, 0, 0);//1列目Right端
        }
        for(i=COLUMN1; i<COLUMN1+ONLEDNUM; i++){
          strip.setPixelColor(i, 255, 0, 0);
        }
...
void setup() {
  ...
  patAllOff();
  Serial.println("KOE_LED System Ver.3 Started!");
